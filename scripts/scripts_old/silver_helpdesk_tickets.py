...
# TRANSFORM & CLEAN: Drop any internal metadata columns
df_clean = df_bronze.select("ticket_id", "email", "issue_type", col("satisfaction_rating").cast("int"))

# DQ CHECK: Ensure satisfaction rating is strictly between 1 and 5
...
df_clean.write.format("delta").mode("overwrite").save("s3a://olist-data/silver/zendesk")
